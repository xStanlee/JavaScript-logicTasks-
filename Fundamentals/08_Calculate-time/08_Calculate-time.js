...
class Majster{
  ...
  khabib(){
    return console.log(`${this.name}: Just send me location.`);
  }
}
...
function unitChanger(obj){
  obj.calcMinutes();
  allMinutes += obj.minutes;
  obj.calcHours();
};

unitChanger(masterAdam);
unitChanger(masterPrzemek);
unitChanger(masterMarcin);

let summaryMinutes = allMinutes%60;
let summaryHours = (allMinutes-summaryMinutes)/60;


function answear(){
  return `Kurs Opanuj JavaScript trwa ${summaryHours} godzin i ${summaryMinutes} minut.
Moduł Adama: ${masterAdam.hours} godzin ${masterAdam.minutes} minut i jest to
Moduł Przemka: ${masterPrzemek.hours} godzin ${masterPrzemek.minutes} minut
Moduł Marcina: ${masterMarcin.hours} godzin ${masterMarcin.minutes} minut
  `
}
